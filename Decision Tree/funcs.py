...
class Probability:
    ...
    def get_distr(data, un_values=[]):
        """
        Creates an array with the probability of each value occurring.
        Parameters
        ----------
        data : numpy-array(int)
            Data whose empirical probabilities are estimated.
        un_values : array-like(int), optional
            Unique values for which probabilities are computed.
        Returns
        ----------
        numpy-array(float)
    ...
    def dist_table(data1, data2):
        """
        Create a table that represents the joint probability of two distributions.
        ...
        numpy matrix
            joint probability of the two distributions
        ...
        if data1.shape == data2.shape:
            ...
            for row in np.transpose(np.array([data1, data2])):
                table[np.where(un_x_values == row[0]), np.where(un_y_values == row[1])] += 1
...
class Itheory:
    @staticmethod
    #    note: if you compare this to scipy.stats.entropy, values differ because
    #    SciPy uses the natural log (entropy in nats) by default, while this uses
    #    log base 2 (entropy in bits). The quantities are the same up to a constant factor.
    def entropy(pmf):
        """
        Calculates entropy
    ...
    def conditional_entropy(table, axis_marginal):
        ...
        axis_marginal : int, 1 or 0
            Determines for which marginal the conditional entropy is computed
            0 - H(`data1`|`data2`)
            1 - H(`data2`|`data1`)
...
class Utility:
    ...
    def less_ent(dist, multiplier=0.01, steps=2, low_border=0.004):
        """
        Changes data such that the entropy decreases.
        """
        size = len(dist)
    ...
    # times: larger -> more shuffling
    # multiplier: larger -> more shuffling
    @staticmethod
    def shuffle_dist(dist, times=10, multiplier=0.4):
        """
        Changes values of a distribution. Depends on the arguments passed.
        ...
        multiplier : float
            How much the function changes the distribution per iteration
    ...
    def shuffle_data(data, chance=0.5, times=10, count_low_boundary=1, count_max_boundary=40):
        ...
        for j in range(0, times):
            for i in range(data.shape[0]):
                if (vals[np.where(uni == data[i])] > count_low_boundary) and (
                    vals[np.where(uni == data[i])] < count_max_boundary
                ):
