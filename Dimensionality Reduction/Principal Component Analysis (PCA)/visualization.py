...
def prepareax(ax, xes=[-7, 7], yes=[-7, 7]):
    """
    Changes plot to a Euclidean view
    """
    x_min = xes[0]
    x_max = xes[1]
    y_min = yes[0]
...
def draw_arrows(ax, origin_points, magnitudes, label, text_size=13, **kargs):
    ...
    magnitudes - directions from origin_points, 2x2 numpy array [[x,x],[y,y]]
    ...
    ax.arrow(
        *origin_points[:, 0],
        *magnitudes[:, 0],
        head_width=0.30 * kargs["w_coef"],
        label=label,
        head_length=0.25 * kargs["w_coef"],
        fc=kargs["color"],
        ec=kargs["color"],
        zorder=kargs["zorder"],
        alpha=kargs["alpha"],
...
def draw_line_points_change(
    ax, points1, points2, color_p, alpha=0.15, linewidth=1, zorder=-1, linestyle="--"
):
    """
    ...
    for indx, point in enumerate(points1):
        ax.add_line(
            lines.Line2D(
                [point[0], points2[indx, 0]],
                [point[1], points2[indx, 1]],
                color=color_p["tl"],
                alpha=alpha,
                linewidth=linewidth,
                linestyle=linestyle,
...
def draw_transformed_axis(
    ...
    for axes in transformation:
        ax.text(axes[0] * gridsize[1], axes[1] * gridsize[1], next(names), fontsize=text_size)
        ax.add_line(
            lines.Line2D(
                [axes[0] * gridsize[0], axes[0] * gridsize[1]],
...
def visualize_transformation(ax, s_t, a_t, name, color_p):
    """
    ...
    color_p - color palette

    Returns:
    n_t  - new transformation
    """
    ax.set_title(name)
    prepareax(ax)
